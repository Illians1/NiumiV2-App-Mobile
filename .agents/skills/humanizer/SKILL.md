...
name: humanizer
description: |
  Remove signs of AI-generated writing from text, including user interface copy.
  Use when editing or reviewing any text to make it sound human-written, AND
  whenever creating, modifying, or reviewing a user interface: screens, components,
  themes, labels, buttons, section titles, banners, empty states, error and
  confirmation messages, onboarding or permission flows, in any language. Applies
  to interface work whether the request is framed as writing, design, or code.
  Composes with design skills rather than replacing them: the design skill owns
  the visuals, this one owns the words. Based on Wikipedia's comprehensive
...
**UI mode.** The task is to create, modify, or review an interface. Apply the pattern
catalogue to the strings a user actually sees: labels, buttons, section titles, banners,
empty states, error and confirmation messages, permission and onboarding explanations.
Leave code, comments, identifiers, and test keys alone.

Judge the set, not the string. A single "Without X, Y" sentence is fine; three in a row
across sibling components is a tell. The same goes for register: formal and informal
address must not alternate between screens, and one screen must not explain itself three
times because three components each carry their own subtitle.

Check the labeling structure, not only the strings. Section headers, group titles, and
subtitles are the interface equivalent of bolded inline headers in prose (§16): a model
writing UI reaches for them to make a screen look organized, and each one in isolation
reads fine. Flag a section label that heads a single row, a label its own first child
restates in other words, and a screen whose only section should have been the screen
title. A group of one is not a group. Count labels against rows before judging: three
headers over five settings is a table of contents, not a page.

Report each finding with its file and line, and apply the fix when you are the one
