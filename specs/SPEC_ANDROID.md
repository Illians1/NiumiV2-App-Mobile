...
**Écran 5 — début du blocage (Lot 6).** Sous le cadran, une section « Blocage des applications » propose deux choix exclusifs : « Maintenant » (défaut, comportement d'origine) et « À partir de », suivi d'une heure ; choisir la seconde ouvre un sélecteur d'heure Material 3 dans une boîte de dialogue, et la ligne affiche l'heure retenue. Le sélecteur s'ouvre sur la dernière heure de début confirmée, à défaut sur 22:00 ; l'annuler sans confirmer laisse « Maintenant » sélectionné, jamais un « À partir de » sans heure (décision du 2026-09-17, étape 24). La ligne affiche l'heure **saisie**, dans la convention 12/24 h du système, comme le cadran du réveil : c'est un champ de saisie, et le sélecteur se rouvre dessus. Seule la phrase de confirmation énonce l'instant obtenu, y compris lors d'un trou d'heure d'été où les deux diffèrent (décision du 2026-09-17, étape 24 : la ligne montrait l'instant quand le choix était valide et la saisie quand il était refusé, ce qui affichait « 15:00 » sous un réveil « 7:00 AM » sur un téléphone en 12 h). Une phrase de confirmation, calculée par `computeBlockingSchedule` à chaque changement de l'une ou l'autre heure et à chaque `ON_RESUME` (recalcul du fuseau, 8), énonce l'instant obtenu : « Tes applications seront bloquées dès l'activation. » ou « Tes applications seront bloquées aujourd'hui, lundi 15 septembre à 22:30 (Europe/Paris). », avec le même libellé relatif, la même date complète et la même règle de trou d'heure d'été que le réveil (`WakeScheduleFormatter`, écrans 5, 6 et 7). Si l'instant n'est pas strictement antérieur au réveil : « L'heure de début du blocage doit être avant ton réveil. Pour bloquer tout de suite, choisis « Maintenant ». », et « Continuer » reste inactif. La dernière heure de début confirmée est mémorisée comme l'heure de réveil (`SetupPreferences`), « Maintenant » restant le défaut tant qu'aucun début différé n'a été confirmé ; confirmer « Maintenant » **retire** la clé, de sorte qu'une session immédiate ne laisse aucun souvenir de début différé. Charte : l'Ambre n'apparaît qu'à la confirmation (charte §9) ; aucune couleur d'alerte pour ce refus, qui n'est pas un incident.
