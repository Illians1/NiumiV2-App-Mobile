 ...
 *
 * [pairedBoxStore] est optionnel : sa seule implémentation vit dans `src/debug` de `:app`
 * jusqu'à l'étape 13, où `RoomPairedBoxStore` la remplace. Absente, le contrôle
 * [ReadinessCheckId.PAIRED_BOX] échoue, ce qui est exact — aucun boîtier ne peut être associé
 * sans dépôt.
 */
data class ReadinessSources(
    val nfcReader: NfcReader,
    val pairedBoxStore: Optional<PairedBoxStore>,
