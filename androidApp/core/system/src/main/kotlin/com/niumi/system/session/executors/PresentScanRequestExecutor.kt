...
/** `PRESENT_SCAN_REQUEST`, best-effort (SPEC_CORE_KMP §6, SPEC_ANDROID §10.5). */
class PresentScanRequestExecutor(
    ...
    override suspend fun execute(
        ...
        val result = notifier.present(snapshot.sessionId)
        if (result is OperationResult.Success) {
            technicalEventLog.log(TechnicalEventType.SCAN_REQUEST_NOTIFIED, snapshot.sessionId)
        }
