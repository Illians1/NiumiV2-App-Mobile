...
**Terminé quand :** l'export ne contient aucune donnée interdite (test), chaque événement du coordinateur apparaît dans le journal, l'écran 12 est accessible. *(**Atteint le 2026-09-13**, validations matérielles comprises. **La cause des doublons d'incidents a été corrigée après coup** — décision de l'utilisateur le même jour : notification et incident ont désormais des cycles de vie distincts dans `SessionReadinessMonitor` (§13.1), et le correctif est validé sur appareil (5 évaluations service coupé → 1 incident, 5 détections au journal, notification republiée à chaque relance). Trois réserves subsistent, consignées dans `ETAPE-16.md` : le texte intégral de l'export n'a pas été relu sur appareil ; `RELEASE_PARTIAL_FAILURE` n'est prouvé qu'en JVM et exige le scan de l'étape 18 ; les deux constats de plateforme de l'étape 15 restent ouverts. Deux constats nouveaux y sont aussi notés : réinstaller l'application révoque le service d'accessibilité (équivalent Play Store non mesuré), et une permission perdue empêche la reprogrammation de l'alarme — §13.1 appliqué, à examiner à l'étape 19.)*
...
- [x] **Écrire les tests de persistance** (`SessionSnapshotMapperTest`, `DirectBootMapperTest`, `DirectBootBlockedPackagesSourceTest`, migration instrumentée), implémenter Room v3, Direct Boot v2 et la branche `ARMED` de la projection. Exporter le schéma 3 et le committer avec le code. *(La branche `ARMED` passe par `isBlockingPending` atteinte via `toSnapshotDto()`, jamais par une recopie de la règle. Trois sentinelles reprises sciemment : `ExportedSchemaTest.CURRENT_VERSION`, le doré de `DirectBootSnapshotJsonTest` (28 → 32 clés) et `TechnicalEventTypeTest`.)*
- [x] **Écrire `BlockingStartPendingIntentSpecsTest`, `BlockingStartHandlerTest` et les tests des deux exécuteurs**, implémenter le programmateur, le receveur, le handler, les exécuteurs et leurs liaisons. Manifeste : le receveur, avec le même commentaire de justification `directBootAware` que le watchdog. *(§17 gagne un cinquième type, `BLOCKING_START_RECEIVED`, journalisé avant toute décision comme `ALARM_RECEIVED` l'est pour le réveil — décision validée le 2026-09-16 : c'est ce qui rend visible une alarme de début ayant survécu à sa session, `CANCEL_BLOCKING_START` étant best-effort. SPEC_ANDROID §17 mise à jour dans le même changement. `BlockingScheduleDto.isImmediate` ajouté à `:shared:core/interop`, seul fichier commun touché, pour que `BLOCKING_STARTED` se distingue d'un `BLOCK_APPLIED` d'activation sans troisième copie de la règle.)*
- [x] **Étendre `PhaseCompletionTest` et `SessionCoordinatorActivationTest`**, implémenter les effets requis. **Sans intersection** : `EffectOutcomes.succeeded` est vacuement vrai pour un kind non produit, donc seule la table change — ce que ce plan anticipait entre parenthèses. Signature et appelants inchangés. Non-régression de l'activation immédiate prouvée (`anImmediateActivationProducesNoBlockingStartEffectAtAll`).
- [x] **Écrire `SessionReconcilerBlockingStartTest`, étendre `SessionReconcilerBootTest`**, implémenter `reconcileBlockingStart`. Ordre prouvé : surveillance §13.1 → garde de permission → relecture → début du blocage → **relecture** → retard du réveil. *(La seconde relecture évite la quatrième occurrence du motif `STALE_REVISION` des étapes 17 et 18. `rescheduleAlarm` sort du corps de classe, plafond detekt oblige ; `ReconcilerSources` passe en arguments nommés après un glissement de douze arguments positionnels.)*
- [x] **Étendre `BlockingProjectionRefresherTest`**, brancher le listener et la relecture du dernier package dans le service. *(La détection de la transition vit dans `PersistedBlockedPackagesProjection`, seul point de passage : elle peut venir du rafraîchissement **ou** d'`APPLY_BLOCKING`, tous deux produits par la même décision. Le service gagne `lastForegroundPackage`, distinct de `lastBlockedPackage`.)*
- [x] **Étendre `ArmSessionUseCaseTest` et `ReadinessDtoMapperTest`**, implémenter le use case à deux heures et le transport du candidat. *(`ActivationFailure.InvalidBlockingSchedule` force un texte dès maintenant dans `SummaryTexts` ; il est provisoire et inatteignable tant que `SummaryViewModel` passe `null`, son verrouillage appartient à l'étape 24.)*
- [x] **Vérifier :** *(989 tests JVM verts le 2026-09-16, `assembleDebug`, `ktlintCheck`, `detekt` et `lintDebug` verts. **83 tests instrumentés verts** sur Xiaomi 25080RABDG / Android 16 / HyperOS OS3.0.302.0 : 73 pour `:core:database`, 10 pour `:app`.)*
...
- [x] **Valider sur appareil** (lire « Particularités de l'appareil de test » d'abord ; **remettre la permission OEM de démarrage automatique à « refusé »**). *(**Déroulée le 2026-09-16 sur Xiaomi 25080RABDG, Android 16, HyperOS OS3.0.302.0.WPPEUXM** — 83 tests instrumentés verts. **Un défaut trouvé, dans le test et non dans le code** : `BlockingStartReceiverInstrumentedTest` attendait le commit du snapshot au lieu de l'exécution de l'effet, alors que le coordinateur persiste avant d'exécuter (SPEC_CORE_KMP §6) — course gagnée en isolé, perdue en suite. Le contrôle `dumpsys alarm` de §9.1 a été **automatisé** (`BlockingStartAlarmVisibilityTest`) plutôt que fait à l'œil : l'instant du début du blocage n'apparaît jamais dans « Next alarm clock information », celui du réveil oui. Le protocole manuel du blocage différé reste reporté à l'étape 24.)*
- [x] **Rédiger `ETAPE-23.md`.**

**Tests manuels : reportés à l'étape 24**, décision prise avec l'utilisateur le 2026-09-16 — sans l'écran 5, armer une vraie session différée exigerait un outil de debug jetable, que CLAUDE.md et la suppression du POC à l'étape 21 déconseillent. Protocole à dérouler alors, consigné dans `ETAPE-23.md` : `dumpsys alarm | grep niumi` → deux alarmes distinctes (`setAlarmClock` du réveil, `setExactAndAllowWhileIdle` du début), aucune « prochaine alarme » système à l'heure du début ; ouvrir une application choisie avant l'heure → **aucun** blocage ; rester dedans à l'heure du début → retour à l'accueil et overlay sans changer de fenêtre ; `am kill` avant l'heure → blocage appliqué à l'heure ; redémarrer avant l'heure sans déverrouiller → `dumpsys alarm` montre l'alarme de début reprogrammée ; redémarrer 20 min après l'heure de début → `MISSED_BLOCKING_START_WINDOW` sur l'écran 12, réveil intact ; scanner avant l'heure de début → `CANCELLED`, plus aucune alarme Niumi.

**Terminé quand :** chaque ligne de SPEC_ANDROID §12.4 a un test ; `RoomBlockedPackagesSource` ne lit `ARMED` que par `isBlockingPending` (grep) ; un `EffectExecutor` existe pour chaque kind (test instrumenté vert) ; l'activation immédiate est prouvée inchangée ; `dumpsys alarm` confirme l'absence d'affichage « prochaine alarme » pour le début du blocage (sinon retour à §9.1 avant de continuer). *(**Atteint le 2026-09-16**, validations matérielles comprises : `EffectExecutorCoverageTest` est vert sur appareil, et le contrôle `dumpsys alarm` est désormais un test automatisé (`BlockingStartAlarmVisibilityTest`) qui prouve que le début du blocage n'atteint jamais le réglage « prochaine alarme ». Deux réserves consignées dans `ETAPE-23.md` : le protocole manuel du blocage différé est reporté à l'étape 24 — aucun essai n'a encore observé le chemin normal déclenché par une vraie alarme à l'heure dite, écran éteint — et la tenue de `setExactAndAllowWhileIdle` en Doze reste à mesurer pour cette alarme, celle du watchdog ne valant pas preuve.)*
...
- [x] **Étendre `WakeTimeViewModelTest`, écrire `BlockingScheduleFormatterTest`**, implémenter le ViewModel, le formateur, les préférences et l'écran 5. Consulter la charte avant de dessiner la section : sobriété, Ambre réservé à la confirmation, aucune couleur d'alerte pour le refus d'antériorité. *(**Deux arbitrages validés le 2026-09-17.** 1. Le `SegmentedButton` Material 3 garde sa place mais perd sa capsule : `itemShape` est construit sur un rayon de 8 dp, la charte §16 rejetant « une succession de grandes capsules très arrondies » — aucune dérogation nécessaire, les deux exigences tiennent ensemble. 2. §15 ne disait pas ce qui arrive quand « À partir de » est choisi sans heure connue : le sélecteur s'ouvre sur la dernière heure confirmée, à défaut 22:00, et l'annuler laisse « Maintenant » — le `ViewModel` ne bascule qu'à la confirmation, de sorte qu'un « À partir de » sans heure est structurellement impossible. SPEC_ANDROID §15 amendée dans le même changement. `SetupPreferences.write` accepte désormais une valeur nulle, qui retire la clé : une écriture comme une autre, donc sous la même garde de déverrouillage, et `DataStoreUnlockGuardTest` l'y soumet explicitement.)*
- [x] **Étendre `SummaryViewModelTest` et `SummaryTextsTest`**, implémenter la route à deux arguments et l'écran 6. *(Le texte provisoire `INVALID_BLOCKING_SCHEDULE_MESSAGE` de l'étape 23 est **supprimé**, pas seulement testé : §15 impose la même phrase aux écrans 5 et 6, et `SummaryTexts` délègue maintenant à `WakeTimeTexts` — un test verrouille cette égalité mot pour mot. Trou trouvé et comblé dans `previewMessage` : un début refusé n'atteint jamais `policy.blockingReasons`, le calcul n'ayant produit aucun instant à soumettre à la politique, et l'écran désactivait donc « Activer ma session » sans rien dire.)*
- [x] **Étendre `ActiveSessionViewModelTest` et `ActiveSessionTextsTest`**, implémenter l'écran 7 (point de vigilance 12 : aucun autre lecteur d'`ARMED` ne change). *(Grep de clôture : les deux seuls lecteurs d'`ARMED` de `:feature:session` sont `ArmSessionUseCase.interpretApplied`, inchangé, et `ActiveSessionTexts.stateLabel`, celui que la vigilance 12 désignait. `isBlockingPending` passe par l'extension interop, jamais par une règle recopiée — grep vert sur tout `androidApp`.)*
- [x] **Vérifier :** *(**876 tests JVM verts le 2026-09-17** — 180 pour `:feature:session`, 377 pour `:core:system`, 124 pour `:core:database`, 81 pour `:app`, 76 pour `:feature:setup`, 38 pour `:feature:ringing` ; `:app:assembleDebug`, `ktlintCheck`, `detekt` et `:app:lintDebug` verts. Quatre remarques de detekt traitées par du code et non par la configuration : `WakeTimeActions` regroupe les trois événements de l'écran 5, `WakeTimeChoice` les deux heures choisies transmises à l'écran 6, `BlockingScheduleFormatter` construit son équivalent sans sorties multiples, et `write` nullable rend le douzième membre de `DataStoreSetupPreferences` inutile.)*
...
- [x] **Valider sur appareil** (protocole ci-dessous, en 12 h et en 24 h, format système changé entre deux essais). *(**Déroulé le 2026-09-17 sur Xiaomi 25080RABDG, Android 16, HyperOS OS3.0.302.0.WPPEUXM** — six essais verts, dont deux sessions réelles. **Un défaut trouvé, dans le code et non dans le test** : un début refusé affichait « Tes applications seront bloquées dès l'activation. » juste au-dessus de son propre message de refus, le faux état que §15 interdit ; la phrase se décide désormais sur le mode choisi, via `WakeTimeUiState.blockingSentence`, couvert par deux tests. **Point de vigilance 13 levé** : le chemin normal est observé déclenché par une vraie alarme — blocage appliqué à **+101 ms** et **+43 ms** de l'instant contractuel, application déjà ouverte renvoyée à l'accueil en **308 ms** sans changement de fenêtre. **Second défaut corrigé le 2026-09-17 :** la ligne d'heure de la section blocage montrait l'instant obtenu quand le choix était valide et la saisie brute quand il était refusé — « 15:00 » sous un réveil « 7:00 AM » en 12 h, et un trou d'heure d'été aurait affiché « 03:00 » pendant que le sélecteur se rouvrait sur 02:30. Elle fait désormais écho à la saisie dans la convention du système, comme le cadran, `WakeScheduleFormatter.formatLocalTime` réutilisant les deux formateurs existants ; SPEC_ANDROID §15 mise à jour. L'`AlarmActivity` vue naître et mourir en une seconde pendant les scans d'annulation avait aussi été signalée comme un défaut — à tort : §10.2 et §10.4 prescrivent d'afficher là la progression du nettoyage, et l'écran y dit « Ton scan est validé. Niumi termine la session. ».)* **Périmètre décidé avec l'utilisateur le 2026-09-17 :** l'interface et le chemin normal ici — parcours 5 → 6 → 7 dans les deux conventions horaires, refus d'antériorité, mémorisation, `dumpsys alarm`, session réelle à +3 min avec une application déjà ouverte, scan avant l'heure, non-régression d'une session immédiate. Les essais lourds du protocole reporté de l'étape 23 (Doze forcé, redémarrage avant l'heure et à +20 min, `am kill`, mesure du retard) restent à l'étape 25, où les scripts `tools/` les outillent — les y rejouer à la main ici les dédoublerait.
- [x] **Rédiger `ETAPE-24.md`.**
